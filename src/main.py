...
def oobe():
	...
	def mesg():
		Pass = f'disk/C/user/{Entr_1.get()}/password.pass'
		Upss = f'disk/C/user/{Entr_1.get()}/upss.pass'
		Patl = f'disk/C/user/{Entr_1.get()}'
		...
		if Entr_1.get() == '':
			...
			tkinter.messagebox.showerror(title="错误",message="更新通道(Default,Beta,TinDOS任选一个)不能为空！")
		elif Entr_3.get() != "Default" or Entr_3.get() != "Beta" or Entr_3.get() != "TinDOS":
			tkinter.messagebox.showerror(title="错误",message="不是有效的更新通道!")
		else:
			INFO_RET = {"UserName":Entr_1.get(),"PassWord":Entr_2.get(),"Update":Entr_3.get()}
...
Start = ttk.Button(root,text='开始',command=sof_list).pack(side='bottom')
Background = tk.Frame(root,bg='#000').pack(fill='both')

root.overrideredirect(True)
w = root.winfo_screenwidth()
h = root.winfo_screenheight()
root.geometry("%dx%d" %(w, h))#root.geometry("600x450+374+182")
root.mainloop()
